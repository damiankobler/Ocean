...
var chatselected = chatrooms.value;
var chatString = "room:chat"+ chatselected;
var channel = socket.channel(chatString, {});     // connect to chat "room"
...
send.addEventListener('click', function (event) {
  if (msg.value.length > 0) { // don't send empty msg.
    sendMessage()
  }
});

chatrooms.addEventListener('change', function (event) {
    changeTopic()
});
